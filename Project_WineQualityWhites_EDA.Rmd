...
由于 quality 与 alcohol 的相关度最高，相关系数有0.44， 所以我们首先来绘制 quality 与 alcohol 的散点图。
...
<br>
从上图看，它们之间的线性关系并不是特别明显。
再来看 quality 与 alcohol 之间的箱图，即查看不同 quality 下的 alcohol 变量的分布情况。
...
<br> 
从这幅箱图中我们发现，alcohol 的中位数并非是随 quality 线性增长的，
...
绘制 quality 与 density 的散点图，增加了抖动和透明度。
...
<br>
quality 与 volatile.acidity 的箱图。
...
<br>
quality 与 total.sulfur.dioxide 的箱图。
...
由于上图中颜色太多，不利于观察，这里将 quanlity 分数切分成三份，
...
```{r echo=FALSE, Plot_Two, fig.width=7, fig.height=6}

p1 <- ggplot(aes(x = alcohol, y = quality), data = wine) +
  geom_jitter(width = 0.05, alpha = 0.3) + 
  geom_smooth(method = 'lm', color = 'blue') + 
  ggtitle("Scatter Plot of Wine Quality by Alcohol") + 
  xlab("alcohol (%)") + 
  ylab("quality")

p2 <- ggplot(aes(x = quality_factor, y = alcohol), data = wine) +
  geom_boxplot() + 
  coord_flip() +
  ggtitle("Box Plot of Wine Quality by Alcohol") + 
  ylab("alcohol (%)") + 
  xlab("quality")
...
第一幅散点图通过抖动和设置透明度的方法，将原本过度重合的点进行区分。
虽然 quality 与 alcohol 之间的相关系数相对较大，但是通过绘制线性拟合的直线，
发现如果只用 alcohol 做线性拟合，并不能很好预测 quality。
然后通过第二幅箱图，我们进一步观察到，其实 quality 并非随 alcohol 的增大而增大的：
在品质小于6时，酒精度越高，品质可能越低，是负相关的；
而在品质大于等于6时， 酒精度越高，品质越有可能提高，是正相关的。
这也解释了为什么第一幅散点图中线性拟合的效果不好了。
